...
9. **coherent CLI lifecycle（S10 #55，已完成）**：platform `CliSession` 统一 clear 与 RunId event projection resume；slash help 从真实注册 catalog 生成。CLI assistant 通过 permission-aware typed manifest/registry selection 运行，每个 initial/resume segment 新建 context/token；JLine SIGINT、二次退出与 UNKNOWN reconciliation 展示都留在 `interfaces/cli`，不下沉 kernel。
