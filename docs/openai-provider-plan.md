...
`https://www.packyapi.com/v1` + `gpt-5` 做真实链路验证。
...
- `ConfigLoader.java`：
  ...
  - **provider 感知默认 model**：OPENAI→`gpt-5`，ANTHROPIC→`claude-sonnet-4-6`
...
- 新增 `OpenAiLlmClientFactory implements LlmClientFactory`：
  ...
  - **gpt-5 注意**：OpenAI 新模型要求 `max_completion_tokens` 且 temperature 只能默认。
...
- **改 `ConfigLoaderTest`**：现有断言假设 anthropic 默认（`missingApiKeyFailsFast` 断言含
  ...
  OPENAI 默认 model=gpt-5、非法 provider fail-fast。
...
- **新增 `OpenAiEndToEndSmokeIT`**：镜像 `EndToEndSmokeIT.java`，
  `@EnabledIfEnvironmentVariable(named="CCLC_API_KEY")`，指向 packyapi + gpt-5，断言返回非空文本；
  归到 `-Psmoke`。
...
- gpt-5 的 `max_completion_tokens` / temperature 限制（见 §2.3，验证步骤兜底）。
