...
`AnthropicStreamingChatModel`，配置只认 `ANTHROPIC_API_KEY` / `AK_MODEL`。
...
- `agentkit-kernel/pom.xml` `<dependencies>`：新增 `langchain4j-open-ai`，紧挨现有
  `langchain4j-anthropic`（`agentkit-kernel/pom.xml:18-21`）。
...
- `ConfigLoader.java`：
  - **provider 解析**：`AK_PROVIDER` → file `provider` → 默认 `OPENAI`；非法值 fail-fast
    （复用 `resolvePermissionMode` 写法）。
  - **apiKey 别名链**：`AK_API_KEY` → `OPENAI_API_KEY` → `ANTHROPIC_API_KEY` → file `apiKey`。
  - **baseUrl 别名链**：`AK_BASE_URL` → `OPENAI_BASE_URL` → `ANTHROPIC_BASE_URL` → file `baseUrl`。
...
### 2.4 组装根（agentkit-cli）

- `AgentKitApplication.java:82`：
...
- **改 `ConfigLoaderTest`**：现有断言假设 anthropic 默认（`missingApiKeyFailsFast` 断言含
  ...
  新增：provider 默认 OPENAI、`AK_PROVIDER` 覆盖、key/baseUrl 别名优先级、
...
- **新增 `OpenAiEndToEndSmokeIT`**：镜像 `EndToEndSmokeIT.java`，
  `@EnabledIfEnvironmentVariable(named="AK_API_KEY")`，指向 packyapi + gpt-5.5，断言返回非空文本；
  归到 `-Psmoke`。
...
- `README.md` 与 `CLAUDE.md`：更新「不在范围内」措辞，补 `AK_PROVIDER` / 通用别名说明。

## 5. 验证

1. 单测：`mvn -pl agentkit-kernel test`（改后的 ConfigLoaderTest、新 LlmClientFactoriesTest 全绿）。
2. 回归：`mvn -pl agentkit-kernel test`（ArchUnit + 426 unit 不破）。
3. 真实链路（packyapi + gpt-5.5）——临时环境变量，**不落盘明文 key**：
   ```powershell
   $env:AK_PROVIDER="openai"
   $env:AK_API_KEY="sk-..."          # 用户提供
   $env:AK_BASE_URL="https://www.packyapi.com/v1"
   $env:AK_MODEL="gpt-5.5"
   mvn -pl agentkit-kernel -Dtest=OpenAiEndToEndSmokeIT -Psmoke "-Dsurefire.skip=true" verify
   ```
   或启 REPL 实测一问一答 + 一次工具调用：
   `mvn -q -pl agentkit-cli -am test-compile exec:java`
...
- 中转站对流式 usage（`stream_options.include_usage`）支持差异——拿不到 usage 不影响主流程，
  仅 token 统计显示 0。
- 默认切 openai 后，原 anthropic 用户需显式 `AK_PROVIDER=anthropic`——已在文档说明。
