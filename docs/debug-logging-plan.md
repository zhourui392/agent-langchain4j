...
> 目标：在 `agentkit` 关键执行链路补齐调试日志，让"一次对话/一次诊断"全程可观测、可回放、可定位。
>
...
|---|---|
| 日志门面 | SLF4J 2.0.13 |
| 日志实现 | Logback 1.5.6（仅 `agentkit-cli` 引入实现） |
| 配置文件 | `agentkit-cli/src/main/resources/logback.xml`：输出 `~/.agentkit/logs/agentkit.log`，按天滚动，保留 7 天 / 200MB；root=INFO，`dev.langchain4j`/`dev.ai4j`=DEBUG |
| 已有日志 | 仅 `agentkit-agent-diagnosis` 的 `interfaces/engine` 包 3 个类共 6 处调用：`DefaultDiagnoseEngine`(4)、`DiagnoseEngineBuilder`(1)、`StreamJsonHistoryParser`(1) |
| 缺口 | **`agentkit-kernel` 业务代码 0 日志、`agentkit-cli` 业务代码 0 日志、诊断工具层 0 日志**。核心主循环、LLM 流式、工具分派、权限决策、持久化全程黑盒 |
...
1. **分层与日志依赖**：`domain` 层**允许依赖 SLF4J 门面（`slf4j-api`），但禁止依赖任何日志实现（logback 等）**。slf4j-api 是日志抽象，依赖它不破坏"domain 不绑定技术实现"的核心诉求（同理于 domain 依赖 JDK）。
   - 该约束在本项目**由构建天然保证**：domain / application / infrastructure 同在 `agentkit-kernel` 模块，其 pom 中 `slf4j-api` 为 compile scope、`logback-classic` 为 **test scope**——main 编译期 classpath 只有门面，拿不到实现。ArchUnit 现状也只拦截 `dev.langchain4j.. / infrastructure / application / interfaces`，未限制 slf4j。
...
## 2. 埋点清单 —— agentkit-kernel（核心引擎）

### 2.1 主循环 `application.AgentExecutor`
文件：`agentkit-kernel/src/main/java/com/anthropic/agentkit/application/AgentExecutor.java`
...
文件：`agentkit-kernel/.../application/ParallelToolDispatcher.java`
...
文件：`agentkit-kernel/.../application/PermissionService.java`
...
文件：`agentkit-kernel/.../application/SystemPromptComposer.java`

| 方法 | 级别 | 埋点内容 |
|---|---|---|
| `compose(workingDir)` | DEBUG | 稳定前缀字符数、动态后缀内容（cwd / git status 摘要 / date）、是否合并了父级 AGENTS.md |

### 2.5 LLM 客户端 `infrastructure.llm.LangChain4jLlmClient`
文件：`agentkit-kernel/.../infrastructure/llm/LangChain4jLlmClient.java`
...
文件：`agentkit-kernel/.../infrastructure/llm/CacheBreakpointStrategy.java`
- DEBUG：在哪几段插入 ephemeral 断点、稳定前缀边界——排查 cache 命中率下降时的首要现场。

### 2.7 持久化 `infrastructure.memory.FileChatMemoryStore`
文件：`agentkit-kernel/.../infrastructure/memory/FileChatMemoryStore.java`
...
- WARN：read-before-write 守卫拦截（未读先写）、检测到外部 mtime 变更——高频报错点，必须可见。
...
## 4. 埋点清单 —— agentkit-cli（接口层）

| 类 | 文件 | 级别 | 埋点内容 |
|---|---|---|---|
| `AgentKitApplication.run()` | `interfaces/cli/AgentKitApplication.java` | INFO | 启动：model、permissionMode、skillsDir 是否启用、注册工具数（**不打 apiKey**） |
| `AgentKitApplication.main()` | 同上 | ERROR | 配置加载失败、致命启动异常（当前是 `System.err`，改为 logger + 保留用户可见提示） |
...
## 5. 埋点清单 —— agentkit-agent-diagnosis（诊断引擎）
...
文件：`agentkit-cli/src/main/resources/logback.xml`
...
2. **分包级别开关**（默认 INFO，排障时调 DEBUG，无需改代码）：
   ```xml
   <logger name="com.anthropic.agentkit.application" level="INFO"/>
   <logger name="com.anthropic.agentkit.infrastructure.tools" level="INFO"/>
   <logger name="com.anthropic.agentkit.infrastructure.llm" level="INFO"/>
   <logger name="com.anthropic.agentkit.infrastructure.diagnosis" level="INFO"/>
   ```
3. **环境变量驱动级别**：用 `${AK_LOG_LEVEL:-INFO}` 让用户一键开 DEBUG，对齐现有 `AK_*` 配置风格。
4. **诊断模块独立 appender（可选）**：`agentkit-agent-diagnosis` 作为进程内 jar 被 agent-web 依赖，宿主可能有自己的 logback。建议该模块**不自带 logback 实现依赖**（保持只依赖 slf4j-api，现状如此），日志配置交由宿主；本仓库测试用 `test` scope 的 logback。
5. **控制台 vs 文件**：当前只有 FILE appender。REPL 是交互式终端，日志写文件即可（避免污染对话输出）；保持现状，**不加 STDOUT appender**。

## 7. 落地步骤（建议顺序）

1. 改 `logback.xml`：加 MDC pattern、分包 logger、`AK_LOG_LEVEL`。（纯配置，先行）
...
6. 自检：跑一次真实对话（`mvn exec:java`）与一次诊断，确认 `~/.agentkit/logs/agentkit.log` 中能按 session/turn 还原完整链路，且无敏感信息泄漏。
